...
这个小组件具备极高的智能化排版和沉浸式的视觉体验：

- **智能多模态展示**：
  - **无比赛时**：展示紧凑的赛程预告列表卡片。
  - **单场比赛直播**：切换为全屏单场直播卡片，享受大尺寸国家队球星头像和发光比分板带来的沉浸式观赛体验。
  - **双场比赛同时直播**：自动无缝切换为**左右分屏双视窗模式**。采用极其克制的极简排版（国家 vs 国家、发光比分板、缩小版球星头像并排），保证在同一屏幕下清爽地追踪两场赛事的实时赛况。
- **动态情绪头像**：自动根据实时比分状况（领先、落后、平局），为每支球队自动切换不同情绪（happy / neutral / sad）的专属球星手绘头像。
- **沉浸式 UI 设计**：包含发光分数面板、深色科幻渐变背景、以及精致的对战排版。
